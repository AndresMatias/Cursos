function conv_ana_ani(x, zx, h, zh, dt, duration = 5, delay = 0)
...
for i = 0:inc:(span + inc);
    try;
      %% Work on signal and system interaction
	    ...
	    subplot(2, 1, 1);
	    cla;
	    pa = plot(rang .+ (round(span / 2) - zx), x, "color", "r", "linewidth", 5, "marker", ".");
	    pb = plot(rang .+ i, h, "color", "b", "linewidth", 5, "marker", ".");
	    axis(axisA);
      grid on;
	    hold off;
      %% Work on convolution animation
	    if (i == 1)
	      subplot(2, 1, 2);
	      plot(rang, zeros(1, numel(rang)), "marker", ".");
	      axis(axisB);
        grid on;
        ...
        for j=1:inc
	      ...
	      subplot(2, 1, 2);
	      cla;
	      plot(rang, shift(addSignals, initI + zx), "color", "g", "linewidth", 5, "marker", ".");
	      %%axis(axisB);
        grid on;
	      hold off;
	    endif
