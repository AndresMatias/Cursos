/**
 * Video 105 al 110 Mircha
 * https://www.youtube.com/watch?v=SBr_EVNErPk&list=PLvq-jIkSeTUZ6QgYYO3MwG9EMqC-KoLXA&index=121&ab_channel=jonmircha    Video 110
 */

/**
 * Cuatro pasos basicos a seguir
 *  1)Crear instancia del objeto(creo que solo es valido para XMLHttpReques)
 *  2)addEventListener
 *  3)Abrir peticion
 *  4)Enviar peticion 
 */
...
/**
 * Ejemplo metodo API Axios + Async-Await (Axios mas funciones asincronas para evitar el uso de promesas then por si llega a ser confuso)
 */
(()=>{
    ...
    async function getData(){
        try{
            let res=await axios.get("assests/user.json"),
               json=await res.data; //Ya devuelve el elemento parseado axios pro lo cual no hace falta conversion a json
               json.forEach(el => {  //Inserto datos de nombre, mail y telefono del archivo json
                const $li=document.createElement("li");
                $li.innerHTML=`${el.name} -- ${el.email} -- ${el.phone}`;
                $fragment.appendChild($li); //Se utiliza funciones autonomas autoejecutadas para que este fragment no entre en conflicto con el fragment de arriba
            });
            $axiosAsync.appendChild($fragment); //Pego info procesada
        }
        catch(err){
            //console.log("Ocurrio un error", err)
            let message=err.response.statusText||"Ocurrio un error";
