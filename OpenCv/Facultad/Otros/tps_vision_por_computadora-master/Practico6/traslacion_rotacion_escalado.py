...
def rotar_trasladar(image, angle,tx,ty,scale):

    (h,w) = image.shape[:2]                                 #Obtiene alto y ancho de la imagen
    center = (w/2, h/2)                                     #Fija el centro de rotación en el centro de la img

    M_r = cv2.getRotationMatrix2D(center,angle,scale)       #Matriz de rotación y escalado

    img_r = cv2.warpAffine(image, M_r, (w,h))               #Imagen rotada y escalada
...
while(1):                                      
    ...
    if k == ord('s'):                                                       #Con 's' aplica transf y sale
        ...
        img_out = rotar_trasladar(img_out, 45, 10, 10, 0.5)                 #Aplico transformación
