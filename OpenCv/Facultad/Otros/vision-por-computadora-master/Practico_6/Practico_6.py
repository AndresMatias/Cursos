#Schwab Urbani, Danel. Visión por computadora: TP6
#! /usr/bin/env python
# -*- coding: utf-8 -*-
...
# Con 'e' Realizo una transformación euclidiana a la imagen recortada y se le agrega un factor de escala s
import cv2
import numpy as np
...
def transformacion_euclidiana(imagen,filas, columnas, traslacion_x, traslacion_y, angulo_rotacion,s):
    filas=filas*s
    columnas=columnas*s
    rad = angulo_rotacion * np.pi/180 
    
    M = np.float32([[s*np.cos(rad), s*np.sin(rad), traslacion_x],  #S debe ser entero 
                    [-s*np.sin(rad), s*np.cos(rad), traslacion_y]])
...
while True:
	...
	if k == ord('g'): #Presiono 'g' para guardar
		...
		s=int(input('Ingrese el factor de escala "s": '))
		img_euclidiana = transformacion_euclidiana(img_nueva, filas, columnas, t_x, t_y, angulo,s) #Asigno imagen al retorno de la función creada
